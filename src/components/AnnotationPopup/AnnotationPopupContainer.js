...
const AnnotationPopupContainer = () => {
  ...
  useEffect(() => {
    ...
    const onMouseLeftUp = (e) => {
      ...
      if (focusedAnnotation) {
        ...
        // Check when isRightClickAnnotationPopupEnabled is disabled, if annotation under mouse is focused or not
        // except the annotation which is placeholder of content edit, even though it on focus, the popup still need to be disbaled.
        const shouldShowPopup = (!isRightClickAnnotationPopupEnabled && annotUnderMouse === focusedAnnotation) && !annotUnderMouse.isInContentEditFocusMode();
        if (shouldShowPopup) {
          setPopupPositionAndShow();
        }
  ...
  const showCalibrateButton =
    canModify && focusedAnnotation.Measure && focusedAnnotation instanceof window.Core.Annotations.LineAnnotation;
