...
const SignatureStylePopup = (props) => {
  ...
  const deleteSignature = (savedSignatureIndex) => {
    signatureToolArray[0].deleteSavedSignature(savedSignatureIndex);

    const isDeletingSelectedSignature = selectedDisplayedSignatureIndex === savedSignatureIndex;
    ...
    } else if (savedSignatureIndex < selectedDisplayedSignatureIndex) {
      dispatch(actions.setSelectedDisplayedSignatureIndex(selectedDisplayedSignatureIndex - 1));
    }
  };

  const deleteInitials = (savedInitialsIndex) => {
    signatureTool.deleteSavedInitials(savedInitialsIndex);

    const isDeletingSelectedSignature = selectedDisplayedInitialsIndex === savedInitialsIndex;
    if (isDeletingSelectedSignature) {
    ...
    } else if (savedInitialsIndex < selectedDisplayedInitialsIndex) {
      dispatch(actions.setSelectedDisplayedSignatureIndex(selectedDisplayedSignatureIndex - 1));
    }
  };
      ...
      <Tabs id="savedSignatures">
        ...
        <TabPanel dataElement={DataElements.SAVED_FULL_SIGNATURES_PANEL}>
          <SavedSignatures
            ...
            deleteHandler={(index) => deleteSignature(index)}
        ...
        <TabPanel dataElement={DataElements.SAVED_INITIALS_PANEL}>
          <SavedSignatures
            ...
            deleteHandler={(index) => deleteInitials(index)}
