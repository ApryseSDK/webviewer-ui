...
const InkSignature = ({
  ...
  const handleFinishDrawingFullSignature = async () => {
    ...
    if (!(await signatureTool.isEmptySignature())) {
      ...
      fullSignaturePathsRef.current = deepCopy(signatureTool.signatureAnnotations[SignatureModes.FULL_SIGNATURE].getPaths());
  ...
  const handleFinishDrawingInitials = async () => {
    const signatureTool = core.getTool('AnnotationCreateSignature');
    const initialsAnnotation = signatureTool.signatureAnnotations[SignatureModes.INITIALS];
    if (initialsAnnotation) {
      // need to deep copy the paths because it will be modified
      // when the annotation is added to the document
      // we want to keep the unmodified paths so that users can keep drawing on the canvas
      initialsPathsRef.current = deepCopy(signatureTool.signatureAnnotations[SignatureModes.INITIALS].getPaths());
  ...
  const handleColorInputChange = (property, value) => {
    ...
    const signatureTool = core.getTool('AnnotationCreateSignature');
    if (signatureTool.signatureAnnotations[SignatureModes.FULL_SIGNATURE]) {
      signatureTool.signatureAnnotations[SignatureModes.FULL_SIGNATURE].StrokeColor = value;
      signatureTool.resizeCanvas(SignatureModes.FULL_SIGNATURE);
    }

    if (signatureTool.signatureAnnotations[SignatureModes.INITIALS]) {
      signatureTool.signatureAnnotations[SignatureModes.INITIALS].StrokeColor = value;
