...
const ImageSignature = ({ isModalOpen, isTabPanelSelected, createSignature }) => {
  ...
  const readFile = file => {
    ...
    fileReader.onload = e => {
      ...
      const validType = acceptedFileTypes.some(type => imageSrc.indexOf(`image/${type}`) !== -1);
    ...
    <React.Fragment>
        ...
        {imageSrc && fileSizeCheck ? (
          ...
          <>
            ...
            <div className={containerClass}>
              {!isMobileDevice && <div className="image-signature-dnd">{t('option.signatureModal.dragAndDrop')}</div>}
              {!isMobileDevice && <div className="image-signature-separator">{t('option.signatureModal.or')}</div>}
              {isMobileDevice && (
                <div className="image-signature-separator">{t('option.signatureModal.selectImage')}</div>
              )}
              <div className="image-signature-upload">
                ...
                />
                <div onClick={() => fileInputRef.current.click()} className="pick-image-button">
                  {t('option.signatureModal.pickImage')}
                </div>
              </div>
              {errorMessage && <div className="image-signature-error">{errorMessage}</div>}
            </div>
          </>
        )}
      </div>
      ...
      >
        ...
        <button
          ...
          disabled={!(isModalOpen && isTabPanelSelected) || imageSrc === null || !fileSizeCheck}
        >
          {t('action.create')}
        </button>
      </div>
