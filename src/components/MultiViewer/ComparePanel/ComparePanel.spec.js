...
describe('ComparePanel component', () => {
  it('Should render correctly when there are no change annotation', () => {
    render(<ComparePanelWithRedux totalChanges={0} />);
    ...
    expect(changeListTitle).toHaveTextContent('(0)');
  });
  it('Should render correctly when there are change annotations', () => {
    const mockItems = {
      '1': [
        {
          'new': {
    ...
    screen.getByRole('checkbox', { name: 'Highlight Changes' });
    const changeListTitle = screen.getByText(/Change List/i);
    expect(changeListTitle).toHaveTextContent('(6)');
