...
test.describe('Create portfolio modal', () => {
  ...
  test('Should be able to create PDF portfolio in a new tab in multi-tab mode', async ({ page }) => {
    ...
    await (await modal.$('.footer .create-portfolio'))?.click();
    await page.waitForTimeout(1000);
    const activeTab = await iframe.evaluate(async () => window.instance.UI.TabManager.getActiveTab().id);
    expect(activeTab).toBe(1);

    // Switch to 1st tab and back to 2nd tab
    await iframe.evaluate(async () => window.instance.UI.TabManager.setActiveTab(0, true));
    await page.waitForTimeout(500);
    await iframe.evaluate(async () => window.instance.UI.TabManager.setActiveTab(1, true));
    await page.waitForTimeout(500);
