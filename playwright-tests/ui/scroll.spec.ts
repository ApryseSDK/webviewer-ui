...
test.describe('Scroll', () => {
  ...
  test('Should have scroll bar for both horizontal and vertical scroll', async ({ page }) => {
    const { iframe, waitForInstance } = await loadViewerSample(page, 'viewing/blank');
    const instance = await waitForInstance();

    const [overflowX, overflowY] = await iframe.evaluate(async () => {
      const scrollViewElementStyle = getComputedStyle(instance.Core.documentViewer.getScrollViewElement());
      return [scrollViewElementStyle.overflowX, scrollViewElementStyle.overflowY];
    });

    expect(overflowX === 'hidden').toBe(false);
    expect(overflowY === 'hidden').toBe(false);
  });
});
